...
export async function getCategory(
...
): Promise<{ code: AJAXErrors; subcategory?: SubCategory }> {
    ...
    return {
        ...
        subcategory: await response.result.json(),
    };
}
...
export async function getSubCategories(
    categoryId: string,
): Promise<{ code: AJAXErrors; data?: SubCategories }> {
...
export async function getSearchCategoryByFilters(
    offset: number,
    categoryId: string,
    searchString: string,
    filters: Filters,
): Promise<{ code: AJAXErrors; products?: any[] }> {
    const request = {};

    if (filters.sortType && filters.sortType !== "default")
    ...
    const response = await ajax.post(
        `search/sort/` + offset + (query !== "?" ? query : ""),
        {
