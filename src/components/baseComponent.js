  ...
  renderElement(context, props, components) {
    ...
    Object.keys(components).map((K) => {
      ...
      if (Array.isArray(components[K])) {
        ...
        const child = components[K].render({
          id: K,
          parent: element,
          showPage: context.showPage,
        });
      ...
      if (Array.isArray(components[key])) {
        ...
        for (let i = 0; i < nowGroup.length; i += 1) {
          if (lastGroup.length > i) {
            // Элемент обновился
            if (
              !compareObjects(nowGroup[i].getProps(), lastGroup[i].getProps())
            ) {
            ...
            if (elementsChanged) {
              ...
              if (propsChanged) {
                const componentElement = newElement.querySelector(
                  `component[data-id="${key}"]`,
                );
            ...
            if (
              ...
              lastGroupChildren[lastGroupChildren.length - 1].after(
                iterComponent,
              );
            }
