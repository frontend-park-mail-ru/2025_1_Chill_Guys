...
class CreateReviewModal extends Tarakan.Component {
    ...
    renderFinished(container: HTMLDivElement) {
        container.style.animation = "0.3s review-modal-showing-animation linear"
    }
            ...
            <div className="review-modal__content">
                ...
                <div className="review-modal__content__form">
                        ...
                        <div className="review-modal__content__form__rating__value">
                            {
                                Array(5).fill(0).map((E, I) =>
                                    <img
                                        className={
                                            this.state.starsHover !== 0 && this.state.starsHover <= I && this.state.starsSelected > I
                                                ? "review-modal__content__form__rating__value__star removed"
                                                : "review-modal__content__form__rating__value__star"
                                        }
                                        src={(this.state.starsHover > I || this.state.starsSelected > I) ? StarFilledIcon : StarIcon}
                                        onMouseOver={() => this.setState({ starsHover: I + 1 })}
                                        onMouseLeave={() => this.setState({ starsHover: 0 })}
                                        onClick={() => this.setState({ starsSelected: this.state.starsHover })}
                                    />
                                )
                            }
                        </div>
                    ...
                    <TextArea
                        rows={5}
                        type="textarea"
                        className="review-modal__content__form__description" title="Комментарий"
                ...
                <div className="review-modal__content__actions">
                    <Button
                        title="Отправить"
                        variant="primary"
                        onClick={() => props.onSend && props.onSend(this.state.description, this.state.starsSelected)}
                        disabled={this.state.description === "" || this.state.starsSelected === 0}
                    />
                </div>
            </div>
        </div>
