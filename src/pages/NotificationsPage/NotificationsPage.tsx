...
class NotificationsPage extends Tarakan.Component {
    ...
    init() {
        ...
        this.subscribe("user", (name: string, value: any) => {
            ...
            if (name === "nots") {
                this.setState({ notifications: value.notifications, unread_count: value.unread_count, readBlock: false });
            }
            if (name === "nof_timer") {
                this.app.store.user.sendAction("getNofitications");
            }
    ...
    render() {
        return <div className="nots-page">
            <Header />
            <main className="nots-page__main">
                ...
                <div className="nots-page__main__notifications">
                    {this.state.notifications.map((notification: Nofitication) =>
                        <div
                            className={
                                notification.isRead
                            ...
                            <div className="nots-page__main__notifications__item__header">
                                <h2 className="nots-page__main__notifications__item__header__h">{notification.title}</h2>
                                {!notification.isRead && <Button
                                    variant="text"
                                    title="Отметить как прочитанное"
                                    className="nots-page__main__notifications__item__header__viewed"
                                    onClick={() => this.app.store.user.sendAction("setVisibleNofitication", notification.id)}
                                />}
                            </div>
                            ...
                            <div className="nots-page__main__notifications__item__date">
                                {(new Date(notification.updatedAt)).toLocaleString("ru-RU")}
                            </div>
                        </div>
                    )}
                </div>
                <InfinityList onShow={() => !this.state.readBlock && this.app.store.user.sendAction("nextNofitications")} />
            </main >
            <Footer />
        </div >
    }
